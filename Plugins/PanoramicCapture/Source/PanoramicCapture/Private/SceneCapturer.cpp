...
void USceneCapturer::ValidateParameters()
{
	...
	{
		FText ErrMsg;
		bool Success = FPaths::ValidatePath(OutputDir, &ErrMsg);
		if (Success) {
			IFileManager::Get().MakeDirectory(*OutputDir, true);
		}
		else {
			FMessageLog(StereoPanoramaLogName).Message(EMessageSeverity::Error, FText::Format(LOCTEXT("ValidationError_InvalidPath", "Destination path {0} is in an invalid format. Error was {1}"), FText::FromString(OutputDir), ErrMsg));
			ErrorFound = true;
		}
	}
